...
#   ******************************************************* #
    ...
    def check_open_doors(self):
        """
        Kiểm tra tủ mở > 5 phút mà door_closed_at vẫn NULL
        → Tức là chưa nhận được tín hiệu CLOSED từ ESP32
        → Gửi mail cảnh báo
        """
        try:
            open_lockers = self.locker_repo.get_open_lockers()

            for locker_id, mssv, timestamp, email in open_lockers:
                try:

                    # Tính số phút đã mở
                    open_time = datetime.strptime(timestamp, "%Y-%m-%d %H:%M:%S")
                    minutes = int((datetime.now() - open_time).total_seconds() / 60)
                    
                    self.send_warning_email(email, mssv, locker_id, minutes)
                    self.locker_repo.mark_door_warned(locker_id)
                    print(f"[DoorMonitor] Đã gửi mail: {mssv} - {locker_id}")
                except Exception as e:
                    print(f"[DoorMonitor] Lỗi gửi mail {mssv}: {e}")

        except Exception as e:
            print(f"[DoorMonitor] Lỗi: {e}")


    def send_warning_email(self, email, mssv, locker_id, minutes):
        """Gửi mail cảnh báo quên đóng tủ"""
        msg = MIMEMultipart("alternative")
        msg["Subject"] = "⚠️ CẢNH BÁO - TỦ LOCKER CHƯA ĐÓNG"
        msg["From"] = f"Smart Locker <{EMAIL_SENDER}>"
        msg["To"] = email
        ...
        <body style="font-family: 'Segoe UI', sans-serif; padding: 20px;">
            <div style="max-width: 500px; margin: auto; background: white; 
                ...
                <h2 style="color: #e53935; text-align: center;">
                    ⚠️ Tủ Chưa Được Đóng!
                </h2>
                <p>Chào bạn (MSSV: <b>{mssv}</b>),</p>
                <p>Tủ <b>{locker_id}</b> của bạn đã mở trong <b>{minutes} phút</b> 
                   mà chưa được đóng lại.</p>
                <div style="background: #fff3cd; padding: 15px; border-radius: 5px;">
                    <b>⚠️ Lưu ý:</b> Vui lòng quay lại đóng tủ để bảo vệ tài sản!
                </div>
            </div>
        </body>
        </html>
